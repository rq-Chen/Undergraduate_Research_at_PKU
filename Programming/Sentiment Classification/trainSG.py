"""trainSG.py - Train skip-gram word vector
...
# For illustration
savePath = './Embedding/'
saveFile = 'sgTest.pickle'
accLim = [0.5, 0.9]
lossLim = [0.3, 0.7]

# For skip-gram
sgEpoch = [5, 15, 25]
sgLoss = np.zeros([len(sgEpoch), max(sgEpoch)])
...
# plt.legend([("%d epochs" % e) for e in sgEpoch])
# plt.title('Loss')
# plt.savefig("%ssgLoss.png" % (savePath))
...
allWV = np.random.rand(len(skGram), nWord, nDim)
# Each column is a word vector
for i in range(len(skGram)):
    for j, word in enumerate(skGram[i].wv.index2word):
        allWV[i, forDict[word], :] = skGram[i].wv.vectors[j,:]
...
for i in range(len(skGram)):
    ...
    lstmHist.append([tmp.history, 'SG',
        {'BatchSize':lstmBatSize, 'LR':lstmLR,
            'Dropout':lstmDrop, 'wvEpoch':sgEpoch[i]}])
...
plt.xlabel('Epoch')
plt.legend(currLegend, loc='lower right')
plt.savefig(savePath + filePre + 'cmpSGAcc.png')
...
plt.xlabel('Epoch')
plt.legend(currLegend, loc='upper right')
plt.savefig(savePath + filePre + 'cmpSGLoss.png')